...
where F: Future<Output = T> + 'static
{
    fn new(future: F) -> RawTask<F, T> {
        RawTask {
            ...
            waker: None,
            u: Ux { future: ManuallyDrop::new(future) },
        }
    }
    ...
    fn poll_task(ptr: *mut RawTask<DummyFuture, ()>) {
        ...
        if raw_task.state != RawTaskState::Running {
            todo!();
        }
        ...
        match pinned.poll(&mut cx) {
            ...
            Poll::Ready(output) => {
                ...
                if raw_task.join_handle_dropped {
                    ...
                    unsafe { (&mut *raw_task.u.future as *mut F).drop_in_place(); }
...
impl<T> Future for JoinHandle<T> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        match &raw_task.state {
            RawTaskState::Ready => {
                raw_task.state = RawTaskState::Closed;
                let mut output = unsafe { &mut raw_task.u.output };
                let output = unsafe { ManuallyDrop::take(&mut output) };
                Poll::Ready(output)
            }
...
impl<T> Drop for JoinHandle<T> {
    fn drop(&mut self) {
        ...
        raw_task.join_handle_dropped = true;
...
fn block_on<T>(future: impl Future<Output = T> + 'static) -> T {
    ...
    assert_eq!(raw_task.state, RawTaskState::Ready);
    raw_task.state = RawTaskState::Closed;

    let mut output = unsafe { &mut raw_task.u.output };
    unsafe { ManuallyDrop::take(&mut output) }
}
